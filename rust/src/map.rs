...
impl <V> MapEntry<V> {
  ...
  pub fn occupied(value: V) -> MapEntry<V> {
  ...
  pub fn take(&mut self) -> Option<V> {
    if self.is_occupied {
      unsafe {
        ...
        let value_ptr: *mut V = &mut self.value;
        let value = value_ptr.read();
        self.value = std::mem::zeroed();
...
impl<V> Default for VecMapU32<V> {
  #[inline]
  fn default() -> Self { Self::new() }
}

impl<V: Hash> Hash for VecMapU32<V> {
...
impl<V> VecMapU32<V> {
  ...
  pub fn with_capacity(capacity: usize) -> Self {
    VecMapU32 {
      n: 0,
      v: VecU32::with_capacity(capacity),
    }
  }
  ...
  pub fn split_off(&mut self, at: usize) -> Self {
    ...
    let first_index = self.v.iter().position(|el| el.is_occupied);
    ...
    other.v.extend((0..start_index).map(|_| MapEntry::vacant()));
    ...
    other.v.extend(self.v[start_index..].iter_mut().map(|el| {
      if el.is_occupied {
        taken += 1;
        return MapEntry::occupied(el.take().unwrap());
      }
      return MapEntry::vacant();
    }));
    other.n = taken;
    self.n -= taken;
  ...
  //
  //   self.n = 0;
  //   let vec: Vec<V> = (self.v.drain().collect());
  //   Drain { iter: vec.enumerate().filter_map(filter) }
  // }
  ...
  pub fn get(&self, key: &u32) -> Option<&V> {
    if (*key as usize) < self.v.len() {
      self.v[*key as usize].as_ref()
    } else {
      None
    }
  }
  ...
  pub fn insert(&mut self, key: u32, value: V) -> Option<V> {
    let len = self.v.len() as u32;
    if len <= key {
      self.v.extend((0..key - len + 1).map(|_| MapEntry::vacant()));
    }
    let mut was = replace(&mut self.v[key as usize], MapEntry::occupied(value));
    if !was.is_occupied {
      self.n += 1;
    }
  ...
  pub fn remove(&mut self, key: &u32) -> Option<V> {
    if key >= &(self.v.len() as u32) {
      return None;
    }
    let result = &mut self.v[*key as usize];
    let was = result.take();
    if was.is_some() {
      self.n -= 1;
    }
...
impl<'a, V> Entry<'a, V> {
  ...
  pub fn remove(self) -> V {
    let index = self.index;
    self.map.remove(&(index as u32)).unwrap()
  }
}

// impl<V: Clone> Clone for VecMapU32<V> {
//   #[inline]
//   fn clone(&self) -> Self {
//     VecMapU32 { n: self.n, v: self.v.clone() }
//   }
//
//   #[inline]
...
impl<V: PartialEq> PartialEq for VecMapU32<V> {
  fn eq(&self, other: &Self) -> bool {
    self.n == other.n && self.iter().eq(other.iter())
  }
}

impl<V: Eq> Eq for VecMapU32<V> {}

impl<V: PartialOrd> PartialOrd for VecMapU32<V> {
  #[inline]
...
impl<V: Ord> Ord for VecMapU32<V> {
  #[inline]
...
impl<V: fmt::Debug> fmt::Debug for VecMapU32<V> {
  fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
    f.debug_map().entries(self).finish()
  }
}

impl<V> FromIterator<(usize, V)> for VecMapU32<V> {
...
// impl<T> IntoIterator for VecMapU32<T> {
//   type Item = (usize, T);
...
impl<'a, T> IntoIterator for &'a VecMapU32<T> {
  type Item = (usize, &'a T);
...
impl<'a, T> IntoIterator for &'a mut VecMapU32<T> {
...
impl<V> Extend<(usize, V)> for VecMapU32<V> {
  fn extend<I: IntoIterator<Item = (usize, V)>>(&mut self, iter: I) {
    for (k, v) in iter {
      self.insert(k as u32, v);
    }
  }
}

impl<'a, V: Copy> Extend<(usize, &'a V)> for VecMapU32<V> {
  fn extend<I: IntoIterator<Item = (usize, &'a V)>>(&mut self, iter: I) {
    self.extend(iter.into_iter().map(|(key, &value)| (key, value)));
  }
}

impl<V> Index<usize> for VecMapU32<V> {
  type Output = V;

  #[inline]
  fn index(&self, i: usize) -> &V {
    self.get(&(i as u32)).expect("key not present")
  }
}

impl<'a, V> Index<&'a usize> for VecMapU32<V> {
  type Output = V;
...
impl<V> IndexMut<usize> for VecMapU32<V> {
  #[inline]
  fn index_mut(&mut self, i: usize) -> &mut V {
    self.get_mut(&(i as u32)).expect("key not present")
  }
}

impl<'a, V> IndexMut<&'a usize> for VecMapU32<V> {
  #[inline]
  fn index_mut(&mut self, i: &usize) -> &mut V {
    self.get_mut(&(*i as u32)).expect("key not present")
  }
...
pub struct IterMut<'a, V: 'a> {
  ...
  n: usize,
  yielded: usize,
  iter: slice::IterMut<'a, MapEntry<V>>
}
...
fn assert_properties() {
  fn vec_map_covariant<T>(map: VecMapU32<&T>) -> VecMapU32<&T> { map }

  fn into_iter_covariant<T>(iter: IntoIter<&T>) -> IntoIter<&T> { iter }

  fn iter_covariant<'i, 'a, T>(iter: Iter<'i, &'a T>) -> Iter<'i, &'a T> { iter }

  fn keys_covariant<'i, 'a, T>(iter: Keys<'i, &'a T>) -> Keys<'i, &'a T> { iter }

  fn values_covariant<'i, 'a, T>(iter: Values<'i, &'a T>) -> Values<'i, &'a T> { iter }
}
...
mod test {
  ...
  // #[test]
  // fn test_move_iter() {
  //   let mut m: VecMapU32<Box<_>> = VecMapU32::new();
  ...
  // #[test]
  // fn test_append() {
  //   let mut a = VecMapU32::new();
  //   a.insert(1, "a");
  //   a.insert(2, "b");
  //   a.insert(3, "c");
  //
  //   let mut b = VecMapU32::new();
  ...
  fn test_show() {
    let mut map = VecMapU32::new();
    let empty = VecMapU32::<i32>::new();

    map.insert(1, 2);
    map.insert(3, 4);

    let map_str = format!("{:?}", map);
    assert!(map_str.as_str() == "{1: 2, 3: 4}" || map_str.as_str() == "{3: 4, 1: 2}");
  ...
  fn test_hash() {
    ...
    let mut x = VecMapU32::new();
    let mut y = VecMapU32::new();
  ...
  fn test_from_iter() {
    ...
    let map: VecMapU32<_> = xs.iter().cloned().collect();

    for &(k, v) in &xs {
      assert_eq!(map.get(&(k as u32)), Some(&v));
    }
  }
  ...
  fn test_entry() {
    ...
    let mut map: VecMapU32<_> = xs.iter().cloned().collect();
    ...
    *v *= 10;
    ...
    let mut entry = map.entry(&10);
    assert_eq!(entry.insert(1000), 1000);
  ...
  fn test_serde() {
    ...
    impls_serde_traits::<VecMapU32<u32>>();
  }
...
// except according to those terms.
  ...
  impl<V> Default for VecMapU8<V> {
    #[inline]
    fn default() -> Self { Self::new() }
  }

  impl<V: Hash> Hash for VecMapU8<V> {
    fn hash<H: Hasher>(&self, state: &mut H) {
      // In order to not traverse the `VecMap` twice, count the elements
      // during iteration.
      let mut count: usize = 0;
  ...
  impl<V> VecMapU8<V> {
    ...
    pub fn with_capacity(capacity: usize) -> Self {
      VecMapU8 {
        n: 0,
        v: VecU8::with_capacity(capacity),
      }
    }
    ...
    pub fn insert(&mut self, key: u8, value: V) -> Option<V> {
      let len = self.v.len() as u8;
      if len <= key {
        self.v.extend((0..key - len + 1).map(|_| None));
      }
      let was = replace(&mut self.v[key as usize], Some(value));
      if was.is_none() {
        self.n += 1;
      }
      was
    ...
    /// use vec_map::VecMap;
    ///
    /// let mut count: VecMap<u8> = VecMap::new();
    ///
    /// // count the number of occurrences of numbers in the vec
    /// for x in vec![1, 2, 1, 2, 3, 4, 1, 2, 4] {
    ///     *count.entry(x).or_insert(0) += 1;
  ...
  impl<'a, V> OccupiedEntry<'a, V> {
    ...
    pub fn remove(self) -> V {
      let index = self.index;
      self.map.remove(&(index as u8)).unwrap()
    }
  }

  impl<V: Clone> Clone for VecMapU8<V> {
    #[inline]
    fn clone(&self) -> Self {
      VecMapU8 { n: self.n, v: self.v.clone() }
    }
  ...
  impl<V: PartialEq> PartialEq for VecMapU8<V> {
    fn eq(&self, other: &Self) -> bool {
      self.n == other.n && self.iter().eq(other.iter())
    }
  }

  impl<V: Eq> Eq for VecMapU8<V> {}

  impl<V: PartialOrd> PartialOrd for VecMapU8<V> {
    #[inline]
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
      self.iter().partial_cmp(other.iter())
    }
  ...
  impl<V: fmt::Debug> fmt::Debug for VecMapU8<V> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
      f.debug_map().entries(self).finish()
    }
  }

  impl<V> FromIterator<(usize, V)> for VecMapU8<V> {
    fn from_iter<I: IntoIterator<Item = (usize, V)>>(iter: I) -> Self {
      let mut map = Self::new();
      map.extend(iter);
      map
...
// }
  ...
  impl<'a, T> IntoIterator for &'a VecMapU8<T> {
    type Item = (usize, &'a T);
    type IntoIter = Iter<'a, T>;
  ...
  impl<'a, T> IntoIterator for &'a mut VecMapU8<T> {
    type Item = (usize, &'a mut T);
    type IntoIter = IterMut<'a, T>;
  ...
  impl<V> Extend<(usize, V)> for VecMapU8<V> {
    fn extend<I: IntoIterator<Item = (usize, V)>>(&mut self, iter: I) {
      for (k, v) in iter {
        self.insert(k as u8, v);
      }
    }
  }

  impl<'a, V: Copy> Extend<(usize, &'a V)> for VecMapU8<V> {
    fn extend<I: IntoIterator<Item = (usize, &'a V)>>(&mut self, iter: I) {
      self.extend(iter.into_iter().map(|(key, &value)| (key, value)));
    }
  }

  impl<V> Index<usize> for VecMapU8<V> {
    type Output = V;

    #[inline]
    fn index(&self, i: usize) -> &V {
      self.get(&(i as u8)).expect("key not present")
    }
  }

  impl<'a, V> Index<&'a usize> for VecMapU8<V> {
    type Output = V;
  ...
  impl<V> IndexMut<usize> for VecMapU8<V> {
    #[inline]
    fn index_mut(&mut self, i: usize) -> &mut V {
      self.get_mut(&(i as u8)).expect("key not present")
    }
  }

  impl<'a, V> IndexMut<&'a usize> for VecMapU8<V> {
    #[inline]
    fn index_mut(&mut self, i: &usize) -> &mut V {
      self.get_mut(&(*i as u8)).expect("key not present")
    }
  ...
  #[allow(dead_code)]
  fn assert_properties() {
    fn vec_map_covariant<T>(map: VecMapU8<&T>) -> VecMapU8<&T> { map }
  ...
  mod test {
    ...
    fn test_hash() {
      ...
      let mut x = VecMapU8::new();
      let mut y = VecMapU8::new();
    ...
    fn test_from_iter() {
      ...
      let map: VecMapU8<_> = xs.iter().cloned().collect();

      for &(k, v) in &xs {
        assert_eq!(map.get(&(k as u8)), Some(&v));
      }
    }
    ...
    fn test_entry() {
      ...
      let mut map: VecMapU8<_> = xs.iter().cloned().collect();
    ...
    fn test_serde() {
      ...
      impls_serde_traits::<VecMapU8<u8>>();
    }
